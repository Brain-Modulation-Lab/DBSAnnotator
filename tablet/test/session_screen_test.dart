...
void main() {
  ...
  final scalePresets =
      ScalePresets.fromJson(readJson('../schema/scale_presets.json'));
  ...
  testWidgets('wizard: file step, then baseline insert lands in the '
    ...
    // Step 3 shows the inserted block in the list.
  ...
  testWidgets('session scales defined in Step 2 become Step-3 rating rows; '
    ...
    // The block appears in the inserted-blocks list.
    expect(find.text('Recording block'), findsOneWidget);
    expect(find.text('No blocks inserted yet.'), findsNothing);
  });
}
