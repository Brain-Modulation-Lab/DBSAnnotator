...
/// (the desktop's preset combos). Run from the `tablet/` dir.
void main() {
  ...
  test('stimulation quick-picks match the desktop preset combos', () {
    ...
    expect(limits.amplitudePresets, [0.0, 1.5, 3.0, 5.0, 7.0, 10.0]);
  ...
  test('step1 / decimals / omitted_tsv fall back to defaults when absent',
      () {
    final json = jsonDecode(File('../schema/limits.json').readAsStringSync())
